...
기능적 요구사항
1. 고객이 예약할 차량 및 날짜를 선택한다.
2. 예약할 차량 및 날짜를 선택하면 해당 예약내용에 대하여 결제한다.
3. 결제가 완료되면 예약이 완료되며, 예약 내역이 렌터카 업체에 전달된다.
3. 예약 내역이 렌터카 업체에 전달되는 동시에, 대여 가능 수량이 변경된다.
4. 업체에서 예약 내역을 확인하여 차량을 준비한다.
5. 차량을 준비 후, 렌트된 상태로 변경된다.
6. 고객이 예약을 취소할 수 있다.
7. 예약을 취소하면 결제도 취소된다.
8. 고객이 차량을 반납하면 대여 가능한 수량이 증가된다.
9. 고객이 예약정보를 중간중간 조회한다.
   ex) 예약시작날짜, 예약종료날짜, 수량 등
9. 업체는 새로운 차량을 등록할 수 있다.
10. 고객은 반납한 차량에 대하여 후기를 작성한다.
11. 고객이 작성한 리뷰는 언제든지 조회할 수 있다.
   ex) 차량, 내용 등

비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 건은 예약이 완료되지 않아야한다. Sync 호출(1)
    2. 예약 완료된 주문건은 대여 가능 수량이 변경되어야 한다. Sync 호출(2)
2. 장애격리
    1. 업체(Store) 기능이 수행되지 않더라도 예약(booking/payment)은 365일 24시간 받을 수 있어야 한다.  Async (event-driven), Eventual Consistency
    2. 예약(booking)이 과중되면 사용자를 잠시동안 받지 않고 잠시후에 하도록 유도한다.  Circuit breaker
3. 성능
    1. 고객이 예약상태를 별도의 고객페이지에서 확인할 수 있어야 한다. CQRS(1)
    2. 고객들에 의해 작성된 후기는 후기페이지에서 확인할 수 있어야 한다. CQRS(2)
